...
export const auditMiddleware: AnyMiddlewareFunction = async ({
  ...
  const requestInfo = auditConfig.includeRequestInfo
...
export const enhancedAuditMiddleware = (
  ...
  return async ({
    ...
    // Extract operation info
    const { tableName, recordId: inputRecordId } = extractOperationInfo(
      path,
      input,
      undefined,
    );
...
export function createAuditMiddleware(
  ...
  return async ({ ctx, next, input }: { ctx: any; next: any; input: any }) => {
    // Skip if audit is disabled
    if (!auditConfig.enabled || !auditDatabaseInstance) {
      return next({ ctx });
    }
    ...
    if (
      auditConfig.excludedTables?.includes(tableName) ||
