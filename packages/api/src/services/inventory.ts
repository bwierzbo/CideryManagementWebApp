...
export class InventoryService {
   ...
   */
  private static validateMaterialTransaction(
    ...
    switch (transaction.materialType) {
      case "apple":
        ...
        if (
          appleTransaction.defectPercentage &&
          (appleTransaction.defectPercentage < 0 ||
            appleTransaction.defectPercentage > 100)
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
      ...
      case "additive":
        ...
        if (
          !additiveTransaction.additiveType ||
          !additiveTransaction.additiveName
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Additive type and name are required for additive transactions",
          });
        }
        ...
        if (
          additiveTransaction.expirationDate &&
          additiveTransaction.expirationDate < new Date()
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
      ...
      case "juice":
        ...
        if (!juiceTransaction.volumeL || juiceTransaction.volumeL <= 0) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Volume is required and must be positive for juice transactions",
          });
        }
        ...
        if (
          juiceTransaction.phLevel &&
          (juiceTransaction.phLevel < 0 || juiceTransaction.phLevel > 14)
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
        ...
        if (juiceTransaction.varietyComposition) {
          const totalPercentage = juiceTransaction.varietyComposition.reduce(
      ...
      case "packaging":
        ...
        if (
          !packagingTransaction.packagingType ||
          !packagingTransaction.packagingName
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Packaging type and name are required for packaging transactions",
          });
        }
  ...
  private static async validateJuiceTransactionReferences(
    ...
    if (transaction.vesselId) {
      const vessel = await tx
        ...
        .from(vessels)
        .where(
          and(eq(vessels.id, transaction.vesselId), isNull(vessels.deletedAt)),
        )
        .limit(1);
    ...
    if (
      transaction.varietyComposition &&
      transaction.varietyComposition.length > 0
    ) {
      const varietyIds = transaction.varietyComposition.map(
        (comp) => comp.appleVarietyId,
      );
  ...
  private static async validateInventoryLevels(
    ...
    const inventoryItem = await tx
      ...
      .from(inventory)
      .where(and(eq(inventory.id, inventoryId), isNull(inventory.deletedAt)))
      .limit(1);
  ...
  static async createInventoryItem(
    ...
    return await db.transaction(async (tx) => {
      try {
        ...
        switch (transaction.materialType) {
          case "apple":
            await this.validateAppleTransactionReferences(
              tx,
              transaction as AppleTransaction,
            );
            break;
          case "juice":
            await this.validateJuiceTransactionReferences(
              tx,
              transaction as JuiceTransaction,
            );
            break;
        ...
        if (!packageId) {
          ...
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Package ID is required for inventory creation. Raw material inventory management needs schema updates.",
          });
        }
      ...
      try {
        ...
        if (inventoryItems.length !== inventoryIds.length) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Some inventory items not found or not at the specified location",
          });
        }
    ...
    try {
      ...
      const stockChecks: StockLevelCheck[] = inventoryItems.map((item) => {
        const availableStock =
          item.currentBottleCount - item.reservedBottleCount;
    ...
    return await db.transaction(async (tx) => {
      try {
        ...
        const inventoryItem = await tx
          ...
          .from(inventory)
          .where(
            and(eq(inventory.id, inventoryId), isNull(inventory.deletedAt)),
          )
          .limit(1);
        ...
        await eventBus.publish(
          "inventory.reserved",
          {
            ...
            availableAfterReservation:
              item.currentBottleCount - newReservedCount,
          },
          userId,
        );
