...
const varietyUpdateSchema = z.object({
  ...
  patch: z.object({
    name: z
      ...
      .optional(),
    fruitType: z.enum(["apple", "pear", "plum", "quince", "berry"]).optional(),
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        if (input.patch.tannin !== undefined)
          updateData.tannin = input.patch.tannin;
        if (input.patch.acid !== undefined) updateData.acid = input.patch.acid;
        if (input.patch.sugarBrix !== undefined)
          updateData.sugarBrix = input.patch.sugarBrix;
        if (input.patch.harvestWindow !== undefined)
          updateData.harvestWindow = input.patch.harvestWindow;
        if (input.patch.varietyNotes !== undefined)
          updateData.varietyNotes = input.patch.varietyNotes;
