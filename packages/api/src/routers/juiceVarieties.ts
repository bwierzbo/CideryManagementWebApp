...
export const juiceVarietiesRouter = router({
  ...
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const whereConditions = [isNull(juiceVarieties.deletedAt)];
  ...
  get: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const variety = await db
        .select()
        .from(juiceVarieties)
        .where(and(eq(juiceVarieties.id, id), isNull(juiceVarieties.deletedAt)))
        .limit(1);
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const existing = await db
        ...
        .from(juiceVarieties)
        .where(
          and(eq(juiceVarieties.name, name), isNull(juiceVarieties.deletedAt)),
        )
        .limit(1);
