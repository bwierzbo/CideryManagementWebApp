...
describe('Purchase Router Invoice Number Integration', () => {
  ...
  beforeEach(async () => {
    ...
    const variety = await db.insert(appleVarieties).values({
      name: 'Test Apple',
  ...
  it('should handle multiple vendors on same date', async () => {
    ...
    expect(result1.purchase.invoiceNumber).toMatch(new RegExp(`^20240315-${testVendorId.replace(/[-]/g, '\\-')}-001$`))
    expect(result2.purchase.invoiceNumber).toMatch(new RegExp(`^20240315-${vendor2Id.replace(/[-]/g, '\\-')}-001$`))
  })
  ...
  it('should preserve total cost calculation with auto-generated invoices', async () => {
    ...
    expect(parseFloat(result.purchase.totalCost)).toBe(400.00)
  ...
  describe('Free Apple Purchase Scenarios', () => {
    it('should handle purchase with null pricePerUnit (free apples)', async () => {
      ...
      expect(result.purchase.invoiceNumber).toMatch(/^20240315-[\w-]+-001$/)
      expect(result.purchase.autoGeneratedInvoice).toBe(true)
      expect(parseFloat(result.purchase.totalCost)).toBe(0.00)
    ...
    it('should handle mixed free and paid apples in same purchase', async () => {
      ...
      expect(parseFloat(result.purchase.totalCost)).toBe(750.00)
      expect(result.items.length).toBe(3)

      // Check individual items
      const paidItem1 = result.items.find(item => parseFloat(item.pricePerUnit || '0') === 2.00)
      const freeItem = result.items.find(item => item.pricePerUnit === null)
      const paidItem2 = result.items.find(item => parseFloat(item.pricePerUnit || '0') === 1.50)
    ...
    it('should handle multiple free apple purchases from same vendor', async () => {
      ...
      expect(parseFloat(result1.purchase.totalCost)).toBe(0.00)
      expect(parseFloat(result2.purchase.totalCost)).toBe(0.00)
    ...
    it('should preserve cost calculation accuracy with free items', async () => {
      ...
      const expectedTotal = 123.456 * 2.3333 + 0.5 * 5.00
      expect(parseFloat(result.purchase.totalCost)).toBeCloseTo(expectedTotal, 2)

      expect(result.items.length).toBe(3)

      // Free item should have null values
      const freeItem = result.items.find(item => parseFloat(item.quantity) === 1000)
      expect(freeItem?.pricePerUnit).toBeNull()
      expect(freeItem?.totalCost).toBeNull()
    })
    ...
    it('should handle zero quantity edge case gracefully', async () => {
      ...
      await expect(caller.purchase.create({
        vendorId: testVendorId,
        purchaseDate,
        items: [{
          appleVarietyId: testAppleVarietyId,
