      ...
      try {
        ...
        const items = await db
          .select({
            ...
            quantity: sql<number>`${packagingPurchaseItems.quantity} - COALESCE(${packagingPurchaseItems.quantityUsed}, 0)`.mapWith(Number),
            quantityPurchased: packagingPurchaseItems.quantity,
