...
export const healthRouter = router({
  ...
  status: publicProcedure.query(async () => {
    ...
    try {
      ...
      const databaseResult = databaseCheck.status === 'fulfilled' ? databaseCheck.value : { status: 'unhealthy', error: 'Database check failed' }
      const systemResult = systemCheck.status === 'fulfilled' ? systemCheck.value : { status: 'unhealthy', error: 'System check failed' }

      // Determine overall status
      const isHealthy = databaseResult.status === 'healthy' && systemResult.status === 'healthy'
    ...
    try {
      const diagnostics: any = {
        ...
        database: {
          ...
          connection_string_prefix: process.env.DATABASE_URL?.substring(0, 20) + '...' || 'not configured'
        }
      }
      ...
      try {
        ...
        diagnostics.database.error = dbError instanceof Error ? dbError.message : 'Database diagnostic failed'
      }
