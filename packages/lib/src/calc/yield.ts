 ...
 * ```
 */
export function calcActualLPerKg(juiceVolumeL: number, inputWeightKg: number): number {
  if (juiceVolumeL < 0) {
  ...
  if (juiceVolumeL > inputWeightKg * 2) {
    throw new Error('Juice volume cannot exceed twice the input weight (physically impossible)')
  }
 ...
 */
export function calculateExtractionEfficiency(actualYield: number, varietyExpectedYield: number): number {
 ...
 */
export function getVarietyYieldRange(varietyName: string): { min: number; max: number; typical: number } {
  const varietyYields: Record<string, { min: number; max: number; typical: number }> = {
    'Honeycrisp': { min: 0.55, max: 0.70, typical: 0.62 },
    'Granny Smith': { min: 0.58, max: 0.72, typical: 0.65 },
    'Gala': { min: 0.52, max: 0.68, typical: 0.60 },
    'Fuji': { min: 0.50, max: 0.65, typical: 0.58 },
    'Northern Spy': { min: 0.60, max: 0.75, typical: 0.68 },
    'Rhode Island Greening': { min: 0.62, max: 0.78, typical: 0.70 },
    'McIntosh': { min: 0.55, max: 0.70, typical: 0.63 },
  ...
  return varietyYields[varietyName] || { min: 0.50, max: 0.70, typical: 0.60 }
}
...
export function calculateWeightedAverageYield(
  ...
  for (const run of pressRuns) {
    if (run.inputWeightKg <= 0 || run.juiceVolumeL < 0) {
      throw new Error('All press runs must have positive weight and non-negative juice volume')
    }
