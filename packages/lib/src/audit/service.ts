...
export function generateAuditChecksum(
  auditEntry: Omit<AuditLogEntry, "checksum">,
): string {
 ...
 */
export function generateDataDiff(
  oldData?: Record<string, any>,
  newData?: Record<string, any>,
): any {
  if (!oldData && !newData) {
...
export function createAuditLogEntry(snapshot: AuditSnapshot): AuditLogEntry {
  ...
  const sanitizedOldData = snapshot.oldData
    ? sanitizeAuditData(snapshot.oldData)
    : undefined;
  const sanitizedNewData = snapshot.newData
    ? sanitizeAuditData(snapshot.newData)
    : undefined;
 ...
 */
export function validateAuditSnapshot(snapshot: AuditSnapshot): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (
    !["create", "update", "delete", "soft_delete", "restore"].includes(
      snapshot.operation,
    )
  ) {
    errors.push(
