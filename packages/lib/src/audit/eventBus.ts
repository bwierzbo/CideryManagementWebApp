...
export type AuditEventType = 'create' | 'update' | 'delete' | 'soft_delete' | 'restore'
...
export class AuditEventBus {
  ...
  async publish(event: AuditEvent): Promise<void> {
    ...
    for (const subscription of this.subscriptions.values()) {
      ...
      if (subscription.tableName && subscription.tableName !== event.tableName) {
        continue
      }
      ...
      try {
        ...
        console.error(`Error in audit event handler ${subscription.id}:`, error)
      }
    }
