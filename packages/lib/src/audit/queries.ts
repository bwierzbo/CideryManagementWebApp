...
import { AuditDatabase, type AuditQueryOptions, type AuditQueryResult } from './database'
...
export const auditQuerySchema = z.object({
  ...
  operation: z.enum(['create', 'update', 'delete', 'soft_delete', 'restore']).optional(),
...
export class AuditQueryService {
  ...
  async queryAuditLogs(input: AuditQueryInput): Promise<AuditQueryResult> {
    ...
    if (validatedInput.startDate && validatedInput.endDate) {
      ...
      const daysDiff = Math.ceil(
        (validatedInput.endDate.getTime() - validatedInput.startDate.getTime()) / (1000 * 60 * 60 * 24)
      )
  ...
  async getTableOperations(
    ...
    const validOperations = ['create', 'update', 'delete', 'soft_delete', 'restore']
  ...
  async getSuspiciousActivity(
    ...
    result.auditLogs.forEach(log => {
      ...
      if (log.reason) {
        ...
        if (hasPattern) {
          suspiciousLogs.push({ ...log, reason: 'Suspicious pattern in reason' })
        }
      }
    ...
    userActivity.forEach((logs, userId) => {
      if (logs.length > maxOperationsPerUser) {
        logs.forEach(log => {
          suspiciousLogs.push({ ...log, reason: `User exceeded ${maxOperationsPerUser} operations` })
        })
      }
    })
    ...
    hourlyDeletes.forEach((count, hour) => {
      if (count > maxDeletesPerHour) {
        result.auditLogs
          ...
          .forEach(log => {
            suspiciousLogs.push({ ...log, reason: `Excessive deletes in hour (${count})` })
          })
      }
    })
