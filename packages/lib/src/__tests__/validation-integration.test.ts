...
describe('Validation Integration Tests', () => {
  ...
  it('should validate transfer operations', () => {
    ...
    expect(() => validateTransfer(transfer, batch, vessel, undefined, 0)).not.toThrow();

    // Test with invalid volume
    const invalidTransfer = { ...transfer, volumeTransferredL: -100 };
    expect(() => validateTransfer(invalidTransfer, batch, vessel, undefined, 0)).toThrow(TransferValidationError);
  });
