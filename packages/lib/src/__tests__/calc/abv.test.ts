...
describe("ABV Calculations", () => {
  describe("calculateAbv", () => {
    it("should calculate ABV correctly with typical cider values", () => {
      ...
      expect(calculateAbv(1.05, 1.0)).toBe(6.56);

      // Strong cider: OG 1.070, FG 1.005 = 8.53% ABV
      expect(calculateAbv(1.07, 1.005)).toBe(8.53);
    ...
    it("should handle edge cases correctly", () => {
      ...
      expect(calculateAbv(1.001, 1.0)).toBe(0.13);
    });
    ...
    it("should throw error for invalid inputs", () => {
      // Negative values
      expect(() => calculateAbv(-1.05, 1.0)).toThrow(
        "Specific gravity readings must be positive numbers",
      );
      expect(() => calculateAbv(1.05, -1.0)).toThrow(
        "Specific gravity readings must be positive numbers",
      );
      ...
      // Zero values
      expect(() => calculateAbv(0, 1.0)).toThrow(
        "Specific gravity readings must be positive numbers",
      );
      expect(() => calculateAbv(1.05, 0)).toThrow(
        "Specific gravity readings must be positive numbers",
      );

      // FG > OG
      expect(() => calculateAbv(1.0, 1.05)).toThrow(
        "Original gravity must be greater than or equal to final gravity",
      );

      // Out of range values
      expect(() => calculateAbv(0.9, 1.0)).toThrow(
        "Original gravity must be between 0.980 and 1.200",
      );
      expect(() => calculateAbv(1.05, 0.9)).toThrow(
        "Final gravity must be between 0.980 and 1.200",
      );
      expect(() => calculateAbv(1.3, 1.0)).toThrow(
        "Original gravity must be between 0.980 and 1.200",
      );
      expect(() => calculateAbv(1.05, 1.3)).toThrow(
        "Final gravity must be between 0.980 and 1.200",
      );
    });
  });
  ...
  describe("calculatePotentialAbv", () => {
    it("should calculate potential ABV assuming complete fermentation", () => {
      expect(calculatePotentialAbv(1.05)).toBe(6.56);
      expect(calculatePotentialAbv(1.07)).toBe(9.19);
      expect(calculatePotentialAbv(1.04)).toBe(5.25);
    });
  });
  ...
  describe("brixToSpecificGravity", () => {
    it("should convert Brix to specific gravity correctly", () => {
      ...
      expect(brixToSpecificGravity(0)).toBe(1.0);

      // Typical apple juice: ~12-15 Brix
      const sg12 = brixToSpecificGravity(12);
      expect(sg12).toBeGreaterThan(1.04);
      expect(sg12).toBeLessThan(1.06);

      const sg15 = brixToSpecificGravity(15);
      expect(sg15).toBeGreaterThan(1.055);
      expect(sg15).toBeLessThan(1.07);

      // Higher Brix should give higher SG
      expect(brixToSpecificGravity(20)).toBeGreaterThan(
        brixToSpecificGravity(15),
      );
    });

    it("should throw error for invalid Brix values", () => {
      expect(() => brixToSpecificGravity(-1)).toThrow(
        "Brix must be between 0 and 50",
      );
      expect(() => brixToSpecificGravity(51)).toThrow(
        "Brix must be between 0 and 50",
      );
    });
  ...
  describe("calculateAttenuation", () => {
    it("should calculate apparent attenuation correctly", () => {
      ...
      expect(calculateAttenuation(1.05, 1.0)).toBe(100);

      // Partial fermentation: OG 1.050, FG 1.010 = 80% attenuation
      expect(calculateAttenuation(1.05, 1.01)).toBe(80);

      // No fermentation: OG 1.050, FG 1.050 = 0% attenuation
      expect(calculateAttenuation(1.05, 1.05)).toBe(0);
    ...
    it("should handle edge cases", () => {
      // Very small difference
      expect(calculateAttenuation(1.001, 1.0)).toBe(100);
    });

    it("should throw error for invalid inputs", () => {
      expect(() => calculateAttenuation(-1.05, 1.0)).toThrow(
        "Specific gravity readings must be positive numbers",
      );
      expect(() => calculateAttenuation(1.05, -1.0)).toThrow(
        "Specific gravity readings must be positive numbers",
      );
      expect(() => calculateAttenuation(0, 1.0)).toThrow(
        "Specific gravity readings must be positive numbers",
      );
      expect(() => calculateAttenuation(1.0, 1.05)).toThrow(
        "Original gravity must be greater than or equal to final gravity",
      );
    });
  });
  ...
  describe("isValidSpecificGravity", () => {
    it("should validate typical specific gravity readings", () => {
      expect(isValidSpecificGravity(1.0)).toBe(true);
      expect(isValidSpecificGravity(1.05)).toBe(true);
      expect(isValidSpecificGravity(1.1)).toBe(true);
      expect(isValidSpecificGravity(0.995)).toBe(true);
      expect(isValidSpecificGravity(1.18)).toBe(true);
    });

    it("should reject invalid specific gravity readings", () => {
      expect(isValidSpecificGravity(0.97)).toBe(false);
      expect(isValidSpecificGravity(1.25)).toBe(false);
      expect(isValidSpecificGravity(-1.05)).toBe(false);
  ...
  describe("ABV Curve Validation", () => {
    it("should follow expected ABV curve progression", () => {
      const testPoints = [
        { og: 1.04, expectedRange: [4.5, 5.5] },
        { og: 1.045, expectedRange: [5.5, 6.5] },
        { og: 1.05, expectedRange: [6.0, 7.0] },
        { og: 1.055, expectedRange: [7.0, 8.0] },
        { og: 1.06, expectedRange: [7.5, 8.5] },
        { og: 1.07, expectedRange: [8.5, 9.5] },
      ];

      testPoints.forEach(({ og, expectedRange }) => {
        const abv = calculateAbv(og, 1.0);
    ...
    it("should show consistent relationship between OG difference and ABV", () => {
      const baseOG = 1.05;
      const testFGs = [1.0, 1.005, 1.01, 1.015, 1.02];
  ...
  describe("ABV Precision and Edge Cases", () => {
    ...
    it("should maintain precision at boundary values", () => {
      ...
      expect(() => calculateAbv(0.98, 0.98)).not.toThrow();
      expect(() => calculateAbv(1.2, 1.2)).not.toThrow();
      expect(() => calculateAbv(1.2, 0.98)).not.toThrow();
    ...
    it("should handle very small gravity differences", () => {
      ...
      const minimalFermentation = calculateAbv(1.001, 1.0);
      expect(minimalFermentation).toBe(0.13); // 0.001 * 131.25 = 0.13125, rounded to 0.13

      const tinyFermentation = calculateAbv(1.0001, 1.0);
    ...
    it("should handle rounding edge cases consistently", () => {
      ...
      const testCases = [
        { og: 1.0038, fg: 1.0, expected: 0.5 }, // 0.0038 * 131.25 = 0.49875 -> 0.5
        { og: 1.004, fg: 1.0, expected: 0.53 }, // 0.004 * 131.25 = 0.525 -> 0.53
        { og: 1.0015, fg: 1.0, expected: 0.2 }, // 0.0015 * 131.25 = 0.196875 -> 0.20
      ];
  ...
  describe("Brix Conversion Precision Tests", () => {
    it("should maintain precision across full Brix range", () => {
      ...
      brixValues.forEach((brix) => {
        ...
        expect(sg).toBeGreaterThanOrEqual(1.0);
        expect(sg).toBeLessThan(1.3); // Reasonable upper bound
  ...
  describe("Attenuation Calculation Edge Cases", () => {
    it("should handle very high attenuation scenarios", () => {
      ...
      const highAttenuation = calculateAttenuation(1.05, 0.995);
    ...
    it("should handle stuck fermentation scenarios", () => {
      ...
      const stuckFermentations = [
        { og: 1.06, fg: 1.03, expectedRange: [50, 60] },
        { og: 1.045, fg: 1.025, expectedRange: [40, 50] },
        { og: 1.05, fg: 1.035, expectedRange: [25, 35] },
      ];
    ...
    it("should maintain mathematical consistency", () => {
      ...
      testCases.forEach(({ og, fg }) => {
        ...
        const manualAttenuation =
          Math.round(((og - fg) / (og - 1.0)) * 100 * 100) / 100;
