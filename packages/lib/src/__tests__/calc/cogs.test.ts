...
describe('COGS Calculations', () => {
  ...
  describe('calculateTotalCogs', () => {
    it('should calculate total COGS correctly with default values', () => {
      ...
      // Total: 5487.50
      expect(totalCogs).toBe(5487.50)
    })

    it('should handle different wastage rates', () => {
      const zeroWastageConfig = { ...defaultConfig, wastageRate: 0 }
      const zeroWastageTotal = calculateTotalCogs(defaultBatchData, zeroWastageConfig)

      const highWastageConfig = { ...defaultConfig, wastageRate: 10 }
      const highWastageTotal = calculateTotalCogs(defaultBatchData, highWastageConfig)
    ...
    it('should throw error for invalid juice volume', () => {
      ...
      expect(() => calculateTotalCogs(invalidBatch, defaultConfig))
        .toThrow('Juice volume must be positive')

      const negativeBatch = { ...defaultBatchData, juiceVolumeL: -100 }
      expect(() => calculateTotalCogs(negativeBatch, defaultConfig))
        .toThrow('Juice volume must be positive')
    })
    ...
    it('should handle edge case of 100% wastage rate', () => {
      ...
      expect(total).toBe(9050.00) // 7500 + 200 + 750 + 600
    })
  })

  describe('calculateCogsComponents', () => {
    it('should return all four COGS components', () => {
      const components = calculateCogsComponents(defaultBatchData, defaultConfig)

      expect(components).toHaveLength(4)
      expect(components.map(c => c.itemType)).toEqual([
        'apple_cost', 'labor', 'overhead', 'packaging'
      ])
    })
    ...
    it('should calculate apple cost with wastage adjustment', () => {
      ...
      expect(appleCost!.amount).toBe(3937.50) // 1500 * 1.05 * 2.50
      expect(appleCost!.quantity).toBe(1575) // 1500 * 1.05
      expect(appleCost!.unitCost).toBe(2.50)
    ...
    it('should calculate labor cost correctly', () => {
      ...
      expect(laborCost!.unitCost).toBe(25.00)
    })
    ...
    it('should calculate packaging cost correctly', () => {
      ...
      expect(packagingCost!.unitCost).toBe(1.20)
    })
  ...
  describe('calculateCostPerLiter', () => {
    it('should calculate cost per liter correctly', () => {
      const costPerL = calculateCostPerLiter(5487.50, 1000)
    ...
    it('should throw error for negative COGS', () => {
      expect(() => calculateCostPerLiter(-100, 1000))
        .toThrow('Total COGS must be non-negative')
    })

    it('should throw error for invalid volume', () => {
      expect(() => calculateCostPerLiter(1000, 0))
        .toThrow('Final volume must be positive')

      expect(() => calculateCostPerLiter(1000, -500))
        .toThrow('Final volume must be positive')
    })
  })
  ...
  describe('calculateCostPerBottle', () => {
    it('should calculate cost per bottle correctly', () => {
      const costPerBottle = calculateCostPerBottle(5487.50, 500)
    ...
    it('should throw error for negative COGS', () => {
      expect(() => calculateCostPerBottle(-100, 50))
        .toThrow('Total COGS must be non-negative')
    })

    it('should throw error for invalid bottle count', () => {
      expect(() => calculateCostPerBottle(1000, 0))
        .toThrow('Bottle count must be positive')

      expect(() => calculateCostPerBottle(1000, -10))
        .toThrow('Bottle count must be positive')
    })
  })
  ...
  describe('calculateGrossMargin', () => {
    ...
    it('should throw error for invalid selling price', () => {
      expect(() => calculateGrossMargin(0, 5))
        .toThrow('Selling price must be positive')

      expect(() => calculateGrossMargin(-10, 5))
        .toThrow('Selling price must be positive')
    })

    it('should throw error for negative COGS', () => {
      expect(() => calculateGrossMargin(15, -5))
        .toThrow('COGS cost must be non-negative')
    })
  })
  ...
  describe('calculateMarkup', () => {
    ...
    it('should throw error for invalid selling price', () => {
      expect(() => calculateMarkup(0, 10))
        .toThrow('Selling price must be positive')
    })

    it('should throw error for invalid COGS cost', () => {
      expect(() => calculateMarkup(15, 0))
        .toThrow('COGS cost must be positive for markup calculation')

      expect(() => calculateMarkup(15, -5))
        .toThrow('COGS cost must be positive for markup calculation')
    })
  })
  ...
  describe('allocateSharedCosts', () => {
    ...
    it('should throw error for negative shared cost', () => {
      ...
      expect(() => allocateSharedCosts(-500, batches))
        .toThrow('Shared cost must be non-negative')
    })

    it('should throw error for empty batch array', () => {
      expect(() => allocateSharedCosts(1000, []))
        .toThrow('At least one batch is required for cost allocation')
    })

    it('should throw error for invalid batch volumes', () => {
      const invalidBatches = [{ batchId: 'batch-1', volumeL: -100 }]
      expect(() => allocateSharedCosts(1000, invalidBatches))
        .toThrow('Batch batch-1 volume must be positive')
    })
  })

  describe('calculateYieldVarianceCostImpact', () => {
    it('should increase cost when actual yield is lower than expected', () => {
      const adjustedCost = calculateYieldVarianceCostImpact(0.6, 0.5, 10.00)
      expect(adjustedCost).toBe(12.0000) // Cost increases by 20%
    })

    it('should decrease cost when actual yield is higher than expected', () => {
      const adjustedCost = calculateYieldVarianceCostImpact(0.6, 0.75, 10.00)
      expect(adjustedCost).toBe(8.0000) // Cost decreases by 20%
    })

    it('should return same cost when yields are equal', () => {
      const adjustedCost = calculateYieldVarianceCostImpact(0.6, 0.6, 10.00)
      expect(adjustedCost).toBe(10.0000)
    })

    it('should round to 4 decimal places', () => {
      const adjustedCost = calculateYieldVarianceCostImpact(0.6123, 0.5456, 10.33)
      const decimalPlaces = adjustedCost.toString().split('.')[1]?.length || 0
      expect(decimalPlaces).toBeLessThanOrEqual(4)
    })

    it('should throw error for invalid yields', () => {
      expect(() => calculateYieldVarianceCostImpact(0, 0.5, 10))
        .toThrow('Yield values must be positive')

      expect(() => calculateYieldVarianceCostImpact(0.6, -0.1, 10))
        .toThrow('Yield values must be positive')
    })

    it('should throw error for negative base cost', () => {
      expect(() => calculateYieldVarianceCostImpact(0.6, 0.5, -10))
        .toThrow('Base cost per liter must be non-negative')
    })
  })
  ...
  describe('getCogsPerformanceCategory', () => {
    it('should categorize COGS performance correctly', () => {
      ...
      expect(getCogsPerformanceCategory(120, 100)).toBe('Significantly Over Budget') // 20% over
    })
    ...
    it('should throw error for invalid COGS values', () => {
      expect(() => getCogsPerformanceCategory(-100, 100))
        .toThrow('COGS values must be positive')

      expect(() => getCogsPerformanceCategory(100, 0))
        .toThrow('COGS values must be positive')
    })
  })
  ...
  describe('calculateInventoryValue', () => {
    it('should calculate inventory value correctly', () => {
      const value = calculateInventoryValue(100, 12.50)
      expect(value).toBe(1250.00)
    })

    it('should handle zero inventory', () => {
      const value = calculateInventoryValue(0, 12.50)
      expect(value).toBe(0)
    })
    ...
    it('should throw error for negative inventory', () => {
      expect(() => calculateInventoryValue(-10, 12.50))
        .toThrow('Inventory bottles must be non-negative')
    })

    it('should throw error for negative COGS per bottle', () => {
      expect(() => calculateInventoryValue(100, -5))
        .toThrow('COGS per bottle must be non-negative')
    })
  })
  ...
  describe('COGS Integration Tests', () => {
    it('should calculate end-to-end batch costing correctly', () => {
      ...
      const config: CostAllocationConfig = {
        ...
        laborRatePerHour: 22.50, // Including benefits
      ...
      const grossMargin = calculateGrossMargin(18.00, costPerBottle) // $18 selling price
    ...
    it('should demonstrate cost impact of poor vs excellent yields', () => {
      ...
      const excellentCogs = calculateTotalCogs(excellentYieldBatch, defaultConfig)
      ...
      const expectedDifference = (2000 - 1200) * 1.05 * 2.50 // Apple cost difference
      expect(costDifference).toBe(expectedDifference)
    })

    it('should show impact of different cost structures on margins', () => {
      const highLaborConfig = { ...defaultConfig, laborRatePerHour: 35.00 }
      const lowOverheadConfig = { ...defaultConfig, overheadRatePerL: 0.25 }

      const highLaborCogs = calculateTotalCogs(defaultBatchData, highLaborConfig)
      const lowOverheadCogs = calculateTotalCogs(defaultBatchData, lowOverheadConfig)
