...
async function backfillVendorVarieties() {
  ...
  try {
    ...
    console.log(`✅ Backfill completed. SQL rows affected: ${result.rowCount || 0}`)
    ...
    console.log(`📊 Total vendor-variety relationships after backfill: ${newCount}`)
    ...
    if (Number(newCount) > 0) {
      ...
      for (const row of sampleResult.rows) {
        const isAutoGenerated = row.notes === 'Auto-generated from existing purchase data'
