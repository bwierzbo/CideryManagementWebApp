...
import { eq, and, desc, asc, isNull, sql, gte, lte, like, gt, lt } from 'drizzle-orm'
...
export function withCursorPagination<T extends PgSelect>(
  ...
  if (params.cursor) {
    try {
      const decodedCursor = JSON.parse(Buffer.from(params.cursor, 'base64').toString())
...
export function generateCursor(packagedAt: Date, id: string): string {
  return Buffer.from(JSON.stringify({ packagedAt: packagedAt.toISOString(), id })).toString('base64')
}
...
export async function getPackagingRunsOptimized(
  ...
  if (filters.batchSearch) {
    countQuery = countQuery.leftJoin(batches, eq(packagingRuns.batchId, batches.id))
  }
  ...
  const formattedResults: PackagingRunListItem[] = results.map(item => ({
    ...
    abvAtPackaging: item.abvAtPackaging ? parseFloat(item.abvAtPackaging.toString()) : null
  }))
 ...
 */
export async function getBatchPackagingRuns(batchIds: string[]): Promise<Map<string, PackagingRunListItem[]>> {
