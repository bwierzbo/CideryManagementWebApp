...
// Types for optimized queries - Discriminated union for bottles and kegs
...
export interface KegFillListItem {
  ...
  vesselId: string;
  vesselName: string | null;
  packagedAt: Date;  // Mapped from filledAt
  packageType: 'keg'; // Always 'keg' for keg fills
  packageSizeML: number; // Mapped from keg capacityML
  packagingMaterialName: string | null; // Keg name/number
  unitsProduced: number; // Always 1 for kegs
  volumeTakenL: number;
  lossL: number;
  lossPercentage: number; // Calculated if loss exists
  status: string | null;
  createdAt: Date;
  // QA summary fields
  fillCheck: null; // Kegs don't have fill check
  abvAtPackaging: number | null;
  qaTechnicianName: null; // Kegs don't track QA technician
  // Keg-specific fields
  kegId: string;
...
export type PackagingRunListItem = BottleRunListItem | KegFillListItem;
 ...
 * UNIFIES bottle runs and keg fills into a single packaging runs list
 */
...
// Re-export unified packaging query
export * from "./unified-packaging-query";
