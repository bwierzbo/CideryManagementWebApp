import { pgTable, uuid, text, decimal, integer, timestamp, boolean, date, index, uniqueIndex, pgEnum } from 'drizzle-orm/pg-core'
...
export const carbonationLevelEnum = pgEnum('carbonation_level', ['still', 'petillant', 'sparkling'])
export const fillCheckEnum = pgEnum('fill_check', ['pass', 'fail', 'not_tested'])
export const packagingRunStatusEnum = pgEnum('packaging_run_status', ['completed', 'voided'])
export const packageSizeTypeEnum = pgEnum('package_size_type', ['bottle', 'can', 'keg'])
export const packagingPhotoTypeEnum = pgEnum('packaging_photo_type', ['fill_level', 'label_placement', 'other'])

// Package size reference table for dropdown population
export const packageSizes = pgTable('package_sizes', {
  ...
  sizePackageTypeUniqueIdx: uniqueIndex('package_sizes_size_package_type_unique_idx').on(table.sizeML, table.packageType),
  packageTypeIdx: index('package_sizes_package_type_idx').on(table.packageType),
...
export const packagingRuns = pgTable('packaging_runs', {
  ...
  batchStatusIdx: index('packaging_runs_batch_status_idx').on(table.batchId, table.status),
...
export const packagingRunPhotos = pgTable('packaging_run_photos', {
  id: uuid('id').primaryKey().defaultRandom(),
  packagingRunId: uuid('packaging_run_id').notNull().references(() => packagingRuns.id, { onDelete: 'cascade' }),
  ...
  packagingRunIdx: index('packaging_run_photos_packaging_run_idx').on(table.packagingRunId),
  uploadedByIdx: index('packaging_run_photos_uploaded_by_idx').on(table.uploadedBy)
}))
...
export const packagingRunsRelations = relations(packagingRuns, ({ one, many }) => ({
  // Core relationships
  batch: one(batches, {
