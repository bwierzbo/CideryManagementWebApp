...
export const auditOperationEnum = pgEnum('audit_operation', ['create', 'update', 'delete', 'soft_delete', 'restore'])
...
export const auditLogs = pgTable('audit_logs', {
  ...
  userTimeIdx: index('audit_logs_user_time_idx').on(table.changedBy, table.changedAt),
  tableTimeIdx: index('audit_logs_table_time_idx').on(table.tableName, table.changedAt)
}))
...
export type AuditOperation = 'create' | 'update' | 'delete' | 'soft_delete' | 'restore'

// Audit log metadata for tracking audit system health
export const auditMetadata = pgTable('audit_metadata', {
  id: uuid('id').primaryKey().defaultRandom(),
