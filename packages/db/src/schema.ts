...
export const basefruitPurchases = pgTable("basefruit_purchases", {
  ...
  autoGeneratedInvoice: boolean("auto_generated_invoice")
    .notNull()
    .default(false),
  notes: text("notes"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
...
export const pressRuns = pgTable("press_runs", {
  ...
  notes: text("notes"),
  totalAppleProcessedKg: decimal("total_apple_processed_kg", {
    precision: 10,
    scale: 3,
  }).notNull(),
...
export const pressItems = pgTable("press_items", {
  ...
  quantityUsedKg: decimal("quantity_used_kg", {
    precision: 10,
    scale: 3,
  }).notNull(),
...
export const juiceLots = pgTable(
  "juice_lots",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    pressRunId: uuid("press_run_id")
...
export const batchCosts = pgTable("batch_costs", {
  ...
  batchId: uuid("batch_id")
    .notNull()
    .references(() => batches.id),
  totalAppleCost: decimal("total_apple_cost", {
    precision: 10,
    scale: 2,
  }).notNull(),
...
export const applePressRuns = pgTable(
  "apple_press_runs",
  {
  ...
  (table) => ({
    ...
    dateStatusIdx: index("apple_press_runs_date_status_idx").on(
      table.scheduledDate,
      table.status,
    ),
...
export const applePressRunLoads = pgTable(
  "apple_press_run_loads",
  {
    ...
    applePressRunId: uuid("apple_press_run_id")
      .notNull()
      .references(() => applePressRuns.id, { onDelete: "cascade" }),
    purchaseItemId: uuid("purchase_item_id")
      .notNull()
  ...
  (table) => ({
    ...
    applePressRunIdx: index("apple_press_run_loads_apple_press_run_idx").on(
      table.applePressRunId,
    ),
    purchaseItemIdx: index("apple_press_run_loads_purchase_item_idx").on(
      table.purchaseItemId,
    ),
    varietyIdx: index("apple_press_run_loads_variety_idx").on(
      table.fruitVarietyId,
    ),

    // Composite index for ordered retrieval within press run
    sequenceIdx: index("apple_press_run_loads_sequence_idx").on(
      table.applePressRunId,
      table.loadSequence,
    ),

    // User attribution indexes
    createdByIdx: index("apple_press_run_loads_created_by_idx").on(
      table.createdBy,
    ),
    updatedByIdx: index("apple_press_run_loads_updated_by_idx").on(
      table.updatedBy,
    ),

    // Unique constraint to prevent duplicate sequences within a press run
    uniqueSequence: uniqueIndex("apple_press_run_loads_unique_sequence").on(
      table.applePressRunId,
      table.loadSequence,
    ),
  }),
...
export const applePressRunsRelations = relations(
  applePressRuns,
  ({ one, many }) => ({
...
export const applePressRunLoadsRelations = relations(
  applePressRunLoads,
  ({ one }) => ({
...
export const tankMeasurements = pgTable(
  "tank_measurements",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    vesselId: uuid("vessel_id")
...
export const tankAdditives = pgTable(
  "tank_additives",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    vesselId: uuid("vessel_id")
...
// Tank Relations
export const tankMeasurementsRelations = relations(
  tankMeasurements,
  ({ one }) => ({
    vessel: one(vessels, {
