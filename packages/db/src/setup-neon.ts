...
async function setupNeonDatabase() {
  ...
  console.log(
    `🔗 Connection URL: ${process.env.DATABASE_URL?.substring(0, 50)}...`,
  );
  ...
  try {
    ...
    console.log(
      `   Database time: ${result.rows?.[0]?.current_time || "Unknown"}`,
    );
    console.log(
      `   PostgreSQL version: ${result.rows?.[0]?.pg_version || "Unknown"}`,
    );
    ...
    if ((tables.rows?.length || 0) === 0) {
      console.log(
        "📋 No tables found. Database is ready for schema migration.",
      );
    } else {
    ...
    console.log("\n📝 Next steps:");
    console.log(
      "   1. Update your .env.local file with your actual Neon connection string",
    );
    ...
    if (error instanceof Error) {
      if (error.message.includes("ENOTFOUND")) {
        ...
        console.error(
          "   - Your username and password are correct in DATABASE_URL",
        );
