...
export class TelemetryCollector {
  ...
  async recordAccessEvents(events: AccessEvent[]): Promise<void> {
    ...
    if (this.eventBuffer.length > this.config.maxEventBufferSize) {
      ...
      console.warn(`⚠️ Telemetry buffer overflow, removed ${excess} oldest events`);
    }
  ...
  async analyzeTrends(days: number = 30): Promise<{
    ...
    const elementTrends: Record<string, 'increasing' | 'decreasing' | 'stable'> = {};
  ...
  private startAggregation(): void {
    ...
    console.log(`📊 Telemetry aggregation started (${this.config.aggregationInterval} min intervals)`);
  }
  ...
  private calculateMetrics(
    ...
    const uniqueElements = new Set(events.map(e => `${e.elementType}:${e.elementName}`));
    ...
    for (const event of events) {
      ...
      typeCounts.set(event.elementType, (typeCounts.get(event.elementType) || 0) + 1);
  ...
  private generateSummary(events: AccessEvent[]): TelemetryExport['summary'] {
    const elementCounts = new Map<string, { name: string; type: string; count: number }>();
    ...
    const highRiskElements = Object.entries(riskAssessment).filter(([, risk]) => risk === 'high');

    if (highRiskElements.length > 0) {
      recommendations.push(`Review high-risk elements: ${highRiskElements.map(([name]) => name).join(', ')}`);
    }

    if (mostAccessedElements.length === 0) {
      recommendations.push('No deprecated element access detected - elements may be safe for removal');
    } else {
      recommendations.push('Monitor deprecated elements for additional 30 days before removal');
    }
  ...
  private generateTrendRecommendations(
    ...
    if (overallTrend === 'increasing') {
      recommendations.push('Overall deprecated element usage is increasing - investigate root causes');
    } else if (overallTrend === 'decreasing') {
      recommendations.push('Overall deprecated element usage is decreasing - good trend');
    }
    ...
    if (increasingElements.length > 0) {
      recommendations.push(`Elements with increasing usage: ${increasingElements.join(', ')}`);
      recommendations.push('Investigate why deprecated elements are being accessed more frequently');
    }
    ...
    if (stableElements.length > 0) {
      recommendations.push('Monitor stable elements for potential removal after cooling-off period');
    }
  ...
  private async aggregateMetrics(): Promise<void> {
    ...
    if (intervalEvents.length > 0) {
      ...
      const maxMetrics = Math.ceil((this.config.retentionDays * 24 * 60) / this.config.aggregationInterval);
  ...
  private async saveExport(data: TelemetryExport, format: 'json'): Promise<void> {
 ...
 */
export function createTelemetryCollector(config?: Partial<TelemetryConfig>): TelemetryCollector {
