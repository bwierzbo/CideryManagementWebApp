...
import {
  ...
  isDeprecatedName,
} from "../migrations/naming-convention";
...
describe("Database Deprecation System", () => {
  ...
  beforeEach(() => {
    ...
    monitor = new DeprecatedMonitor(mockDb, telemetryCollector, alertSystem, {
      enabled: false,
    });
    rollbackManager = new RollbackManager(mockDb, {
  ...
  describe("Safety Checks", () => {
    const mockElement: DeprecatedElement = {
      ...
      migrationSql:
        'ALTER TABLE "public"."test_table" RENAME TO "test_table_deprecated_20250928_unu";',
      rollbackSql:
        'ALTER TABLE "public"."test_table_deprecated_20250928_unu" RENAME TO "test_table";',
    };
      ...
      mockDb.execute.mockImplementation((query) => {
        ...
        if (queryStr.includes("foreign_key")) {
          return Promise.resolve({
            rows: [{ constraint_name: "fk_test", table_name: "users" }],
          });
        }
  ...
  describe("Deprecation Planning", () => {
    it("should create a valid deprecation plan", async () => {
      ...
      expect(plan.elements[0].deprecatedName).toMatch(
        /^user_preferences_deprecated_\d{8}_unu$/,
      );
...
describe("Backup Validator", () => {
  ...
  it("should validate backup requirements", () => {
    ...
    } = require("../migrations/backup-validator");
...
export const TestHelpers = {
  createMockElement: (
    overrides: Partial<DeprecatedElement> = {},
  ): DeprecatedElement => ({
    type: "table",
    originalName: "test_table",
    deprecatedName: "test_table_deprecated_20250928_unu",
