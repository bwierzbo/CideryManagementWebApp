...
export async function performSafetyChecks(
  ...
  console.log(`✅ Safety checks completed: ${results.filter(r => r.passed).length} passed, ${results.filter(r => !r.passed).length} failed`);
...
async function checkElementExists(
  ...
  for (const element of elements) {
    ...
    try {
      switch (element.type) {
        case 'table':
          ...
          `;
          const tableResult = await db.execute(sql.raw(tableQuery), [element.schema, element.originalName]);
        ...
        case 'column':
          ...
          `;
          const columnResult = await db.execute(sql.raw(columnQuery), [element.schema, tableName, columnName]);
        ...
        case 'index':
          ...
          `;
          const indexResult = await db.execute(sql.raw(indexQuery), [element.schema, element.originalName]);
          exists = (indexResult.rows[0]?.count as number) > 0;
          break;

        case 'constraint':
          const [constraintTable, constraintName] = element.originalName.split('.');
          const constraintQuery = `
            SELECT COUNT(*) as count
            FROM information_schema.table_constraints
            WHERE table_schema = $1 AND table_name = $2 AND constraint_name = $3
          `;
          const constraintResult = await db.execute(sql.raw(constraintQuery), [element.schema, constraintTable, constraintName]);
    ...
    results.push({
      ...
      severity: 'critical',
      message: exists && !errorMessage
        ? `Element exists and can be deprecated`
        : errorMessage || `Element does not exist: ${element.type} ${element.originalName}`,
...
async function checkConfidenceScore(
  ...
  for (const element of elements) {
    ...
    results.push({
      ...
      details: { confidence, threshold: minimumConfidence, element: element.originalName },
      timestamp: new Date().toISOString(),
    });
  }
...
async function checkDependencies(
  ...
  for (const element of elements) {
    const highImpactDeps = element.dependencies.filter(dep => dep.impact === 'high');
    ...
    results.push({
      ...
      details: { dependencies: element.dependencies, highImpact: highImpactDeps },
      timestamp: new Date().toISOString(),
    });
  }
...
async function checkForeignKeySafety(
  ...
  for (const element of elements) {
    ...
    try {
      ...
      const fkResult = await db.execute(sql.raw(fkQuery), [element.originalName, element.schema]);
      const referencingTables = fkResult.rows;

      const passed = referencingTables.length === 0 || config.allowRiskyOperations;
...
async function checkNamingCollision(
  ...
  for (const element of elements) {
      ...
      switch (element.type) {
        case 'table':
          const tableQuery = `
            ...
            WHERE table_schema = $1 AND table_name = $2
          `;
          const tableResult = await db.execute(sql.raw(tableQuery), [element.schema, element.deprecatedName]);
        ...
        case 'index':
          const indexQuery = `
            ...
            WHERE schemaname = $1 AND indexname = $2
          `;
          const indexResult = await db.execute(sql.raw(indexQuery), [element.schema, element.deprecatedName]);
    ...
    results.push({
      ...
      message: !hasCollision && !errorMessage
        ? 'No naming collision detected'
        : errorMessage || `Deprecated name already exists: ${element.deprecatedName}`,
...
async function checkEnvironmentSafety(
  ...
  for (const element of elements) {
    ...
    const passed = !isProduction || riskLevel === 'low' || config.allowRiskyOperations;
...
async function checkBackupValidation(
  ...
  if (requiresBackup) {
    ...
    const backupAvailable = process.env.DATABASE_BACKUP_URL || process.env.PGDUMP_PATH;
...
async function checkTransactionSafety(
  ...
  const hasLongOperations = elements.some(e =>
    e.type === 'table' && e.dependencies.length > 5
  );
 ...
 */
function assessElementRisk(element: DeprecatedElement): 'low' | 'medium' | 'high' {
...
Overall status: ${overallPassed ? '✅ PASSED' : '❌ FAILED'}
  ...
  if (failed.length > 0) {
    recommendations.push('Consider using strictMode: false for non-critical issues');
  }
