...
jest.mock('@/components/pressing', () => ({
  PressRunCompletion: () => <div data-testid="press-run-completion">PressRunCompletion</div>,
}))
...
describe('PressingPage Navigation Tests', () => {
  ...
  describe('Completed Press Run Navigation', () => {
    test('renders completed press run cards with proper navigation attributes', () => {
      ...
      const cards = screen.getAllByRole('button').filter(card =>
    ...
    test('navigates to press run detail page when card is clicked', async () => {
      ...
      const pressRunCard = screen.getByLabelText(/View press run details for Test Press Run 1/)

      fireEvent.click(pressRunCard)

      await waitFor(() => {
        expect(mockRouter.push).toHaveBeenCalledWith('/pressing/test-press-run-1')
      })
    })
  ...
  describe('Active Press Run Navigation', () => {
    test('active press runs do not have navigation functionality', () => {
      ...
      const viewButtonsInActiveSection = activeSection?.querySelectorAll('[data-testid="view-press-run"]')
  ...
  describe('Error Handling', () => {
    test('handles loading state gracefully', () => {
      ...
      expect(screen.getByText('Loading active press runs...')).toBeInTheDocument()
    })
    ...
    test('handles empty data gracefully', () => {
      ...
      expect(screen.getByText('No completed press runs found')).toBeInTheDocument()
    })
  })
