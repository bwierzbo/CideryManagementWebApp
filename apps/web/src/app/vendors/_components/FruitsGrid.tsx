...
function EditableCell({
  ...
  const getOptions = () => {
    ...
    if (["tannin", "acid", "sugarBrix"].includes(columnId))
  ...
  const getDisplayValue = () => {
    ...
    if (["tannin", "acid", "sugarBrix"].includes(columnId))
      return getIntensityLabel(value);
...
export function FruitsGrid({ userRole }: FruitsGridProps) {
  ...
  const [pendingUpdates, setPendingUpdates] = useState<
    Map<string, Partial<FruitVariety>>
  >(new Map());
  const [savingCells, setSavingCells] = useState<Set<string>>(new Set());
  const [originalValues, setOriginalValues] = useState<Map<string, any>>(
    new Map(),
  );
  ...
  const varieties = useMemo(() => {
    ...
    return allVarieties.filter(
      (variety) => variety.fruitType === fruitTypeFilter,
    );
  ...
  useEffect(() => {
    if (debouncedUpdates.size > 0) {
      debouncedUpdates.forEach((updates, varietyId) => {
        ...
        if (updates.fruitType !== undefined)
          patch.fruitType = updates.fruitType;
        if (updates.ciderCategory !== undefined)
          patch.ciderCategory = updates.ciderCategory;
        if (updates.tannin !== undefined) patch.tannin = updates.tannin;
        if (updates.acid !== undefined) patch.acid = updates.acid;
        if (updates.sugarBrix !== undefined)
          patch.sugarBrix = updates.sugarBrix;
        if (updates.harvestWindow !== undefined)
          patch.harvestWindow = updates.harvestWindow;
        if (updates.varietyNotes !== undefined)
          patch.varietyNotes = updates.varietyNotes;
        ...
        cell: ({ getValue, row, column }) => {
          ...
          return (
            <EditableCell
              ...
              onSave={(value) =>
                handleCellSave(row.original.id, columnId, value)
              }
