...
const juiceLineSchema = z.object({
  ...
  volume: z.number().min(0.1, "Volume must be at least 0.1").max(10000, "Volume cannot exceed 10,000").optional(),
  unit: z.enum(['gallons', 'liters'], { message: "Please select a unit" }),
  specificGravity: z.number().min(0.9, "Specific gravity must be at least 0.9").max(1.2, "Specific gravity cannot exceed 1.2").optional(),
  ph: z.number().min(0, "pH must be at least 0").max(14, "pH cannot exceed 14").optional(),
...
export function JuiceTransactionForm({
  ...
  const vendorJuices = React.useMemo(() =>
    vendorJuicesData?.varieties.filter((v: any) => v.varietyType === 'juice') || [],
    [vendorJuicesData]
  )
    ...
    setLines(prevLines => {
      const newLines = prevLines.map(line => {
        ...
        const newLine = {
          ...line,
          isValid,
          validationError: isValid ? undefined : "This juice is not available for the selected vendor"
        }
  ...
  const calculateLineTotal = (volume: number | undefined, price: number | undefined, totalCost: number | undefined) => {
  ...
  const onFormSubmit = (data: JuiceTransactionFormData) => {
    ...
    if (hasInvalidLines) {
      addNotification('error', 'Invalid Juices', 'Please fix juice selections that are not available for the selected vendor')
      return
    }
      ...
      const items = data.lines
        ...
        .map(line => {
          ...
          return {
            ...
            totalCost: line.totalCost || (line.unitCost ? line.volume! * line.unitCost : undefined),
          }
        })

      if (items.length === 0) {
        addNotification('error', 'Incomplete Form', 'Please add at least one juice with volume')
        return
      }
  ...
  const handleUnitCostChange = (index: number, value: number | undefined) => {
    ...
    if (value && newLines[index].volume) {
      newLines[index].totalCost = parseFloat((value * newLines[index].volume!).toFixed(2))
    }
    setLines(newLines)
  ...
  const handleTotalCostChange = (index: number, value: number | undefined) => {
    ...
    if (value && newLines[index].volume) {
      newLines[index].unitCost = parseFloat((value / newLines[index].volume!).toFixed(2))
    }
    setLines(newLines)
    ...
    <>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Record new juice purchases from vendors</CardDescription>
        </CardHeader>
            ...
            <div>
                  ...
                  <div key={index} className="border rounded-lg p-4">
                    ...
                    <div className="hidden lg:grid lg:grid-cols-8 gap-4">
                      <div className="lg:col-span-2">
                        ...
                        <Select
                          ...
                          disabled={!selectedVendorId || vendorJuices.length === 0}
                        >
                            ...
                            {vendorJuices.map((juice: any) => (
                              <SelectItem key={juice.id} value={juice.id}>
                                <div>
                                  ...
                                  {juice.description && (
                                    <div className="text-xs text-gray-500">{juice.description}</div>
                                  )}
                                </div>
                              </SelectItem>
                            ))}
                        ...
                        {selectedVendorId && vendorJuices.length === 0 && (
                          <p className="text-xs text-gray-500 mt-1">
                            No juices linked to this vendor. Please link juices on the Vendors page first.
                          </p>
                        )}
                      </div>
                      <div>
                        <Label>Volume <span className="text-gray-500 text-sm">(Required)</span></Label>
                        <Input
                          type="number"
                          step="0.1"
                          value={line.volume || ''}
                          ...
                          onChange={(e) => {
                            const newLines = [...lines]
                            newLines[index].volume = e.target.value ? parseFloat(e.target.value) : undefined
                            // Recalculate total if unit cost is set
                            if (newLines[index].unitCost && newLines[index].volume) {
                              newLines[index].totalCost = parseFloat((newLines[index].unitCost! * newLines[index].volume!).toFixed(2))
                            }
                            setLines(newLines)
                            setValue(`lines.${index}.volume`, newLines[index].volume)
                            setValue(`lines.${index}.totalCost`, newLines[index].totalCost)
                          }}
                        />
                      </div>
                      <div>
                        <Label>Unit</Label>
                        <Select value={line.unit} onValueChange={(value: 'gallons' | 'liters') => {
                          const newLines = [...lines]
                      ...
                      <div>
                        <Label>SG <span className="text-gray-500 text-sm">(Optional)</span></Label>
                        <Input
                          type="number"
                          step="0.001"
                          ...
                          onChange={(e) => {
                            const newLines = [...lines]
                            newLines[index].specificGravity = e.target.value ? parseFloat(e.target.value) : undefined
                            setLines(newLines)
                            setValue(`lines.${index}.specificGravity`, newLines[index].specificGravity)
                          }}
                        />
                      </div>
                      <div>
                        <Label>pH <span className="text-gray-500 text-sm">(Optional)</span></Label>
                        <Input
                          type="number"
                          step="0.1"
                          value={line.ph || ''}
                          ...
                          onChange={(e) => {
                            const newLines = [...lines]
                            newLines[index].ph = e.target.value ? parseFloat(e.target.value) : undefined
                            setLines(newLines)
                    ...
                    <div className="lg:hidden space-y-4">
                      <div className="flex items-center justify-between">
                        <h4 className="font-medium text-gray-900">Juice #{index + 1}</h4>
                        {lines.length > 1 && (
                          <Button
                            type="button"
                            variant="outline"
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <Label>Specific Gravity <span className="text-gray-500 text-sm">(Optional)</span></Label>
                          <Input
                            type="number"
                            step="0.001"
                            value={line.specificGravity || ''}
                      ...
                      <div>
                        ...
                        <div className="text-xl font-semibold text-blue-600">
                          {(line.volume != null && line.volume > 0) && (line.unitCost || line.totalCost)
                            ? `$${calculateLineTotal(line.volume, line.unitCost, line.totalCost)}`
              ...
              <div className="flex justify-end mt-4">
                <div className="text-right">
                  ...
                  <p className="text-2xl font-bold text-blue-600">${calculateGrandTotal()}</p>
                </div>
              </div>
            </div>
