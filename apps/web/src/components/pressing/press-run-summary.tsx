...
export function PressRunSummary({
  ...
  const vendorVarietySummary =
    loads && loads.length > 0
      ? loads.reduce(
          (acc, load) => {
            const vendor =
              load.vendorName || pressRun.vendorName || "Unknown Vendor";
            ...
            if (load.purchaseItemId) {
              const existingItem = acc[key].purchaseItems.get(
                load.purchaseItemId,
              );
              if (existingItem) {
                ...
                if (
                  !existingItem.originalPurchaseQuantityKg &&
                  load.purchaseItemOriginalQuantityKg
                ) {
                  existingItem.originalPurchaseQuantityKg =
                    load.purchaseItemOriginalQuantityKg;
                  existingItem.originalPurchaseQuantity =
                    load.purchaseItemOriginalQuantity;
                  existingItem.originalPurchaseUnit =
  ...
  if (vendorVarietySummary && typeof vendorVarietySummary === "object") {
    Object.keys(vendorVarietySummary).forEach((key) => {
      ...
      if (
        summary &&
        summary.brixMeasurements &&
        summary.brixMeasurements.length > 0
      ) {
        summary.averageBrix =
          summary.brixMeasurements.reduce(
            (sum: number, brix: number) => sum + brix,
            0,
          ) / summary.brixMeasurements.length;
      }
    });
  }
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {vendorVarietySummary &&
            Object.keys(vendorVarietySummary).length > 0 ? (
              Object.entries(vendorVarietySummary).map(([key, summary]) => (
                <div key={key} className="border rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium">
                        {summary.totalOriginalWeight.toFixed(1)}{" "}
                        {summary.originalUnit}
                      </p>
                      <p className="text-xs text-gray-500">
                        {summary.totalWeightKg.toFixed(1)} kg •{" "}
                        {(summary.totalWeightKg * 2.20462).toFixed(1)} lbs
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="font-medium">
                        {(
                          (summary.totalWeightKg / totalWeightKg) *
                          100
                        ).toFixed(1)}
                        %
                      </p>
                  ...
                  {summary.conditions.length > 0 && (
                    <div className="mt-2">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {[...new Set(summary.conditions)].map(
                          (condition, index) => (
                            <Badge
                              key={`${condition}-${index}`}
                              variant="secondary"
                  ...
                  {showInventoryCheckboxes &&
                    summary.purchaseItems.size > 0 && (
                      <div className="mt-3 pt-3 border-t">
                        <p className="text-xs text-gray-600 mb-2">
                          Mark transaction as depleted and archive:
                        </p>
                        <div className="space-y-2">
                          {Array.from(summary.purchaseItems.values()).map(
                            (item: any, index: number) => {
                              // Calculate estimated remaining weight
                              const originalKg =
                                item.originalPurchaseQuantityKg || 0;
                                ...
                                >
                                  <div className="flex items-center justify-between text-sm">
                                    ...
                                    <span className="text-gray-500">
                                      Used: {item.originalWeight.toFixed(1)}{" "}
                                      {item.originalWeightUnit}
                                    </span>
                                  </div>
                                  {item.originalPurchaseQuantityKg && (
                                    <div className="text-xs text-gray-600 grid grid-cols-2 gap-2">
                                      <div>
                                        ...
                                        {item.originalPurchaseQuantity?.toFixed(
                                          1,
                                        )}{" "}
                                        {item.originalPurchaseUnit}
                                      </div>
                                      <div>
