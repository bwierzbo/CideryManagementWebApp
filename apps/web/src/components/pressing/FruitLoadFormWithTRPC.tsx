...
const fruitLoadSchema = z.object({
  ...
  weight: z
    ...
    .max(10000, "Weight cannot exceed 10,000")
    .optional(),
...
export function FruitLoadFormWithTRPC({
  ...
  const { data: appleVarieties, isLoading: varietiesLoading } =
  ...
  const form = useForm<FruitLoadFormData>({
    ...
    defaultValues: {
      ...
      weight: editingLoad
        ? editingLoad.originalWeightUnit === "lb"
          ? parseFloat(editingLoad.originalWeight || "")
          : undefined
        : undefined,
      brixMeasured: editingLoad?.brixMeasured
        ? parseFloat(editingLoad.brixMeasured)
        : undefined,
      phMeasured: editingLoad?.phMeasured
        ? parseFloat(editingLoad.phMeasured)
        : undefined,
      appleCondition: editingLoad?.appleCondition || undefined,
      defectPercentage: editingLoad?.defectPercentage
        ? parseFloat(editingLoad.defectPercentage)
        : undefined,
  ...
  useEffect(() => {
    if (watchedWeight && watchedUnit) {
      const weightKg =
        watchedUnit === "kg"
          ? watchedWeight
          : convertWeight(watchedWeight, "lbs", "kg");
  ...
  useEffect(() => {
    if (editingLoad) {
      form.reset({
        ...
        weight:
          editingLoad.originalWeightUnit === "lb"
            ? parseFloat(editingLoad.originalWeight || "0")
            : undefined,
        brixMeasured: editingLoad.brixMeasured
          ? parseFloat(editingLoad.brixMeasured)
          : undefined,
  ...
  useEffect(() => {
    if (editingLoad && purchaseLines?.items) {
      const purchaseItem = purchaseLines.items.find(
        (item) => item.purchaseItemId === editingLoad.purchaseItemId,
      );
      if (purchaseItem) {
  ...
  const filteredPurchaseLines =
    purchaseLines?.items.filter(
      (line) =>
  ...
  const handleSubmit = (data: FruitLoadFormData) => {
    ...
    const variety = appleVarieties?.appleVarieties.find(
      (v) => v.id === data.fruitVarietyId,
    );
    const weightKg =
      data.weightUnit === "kg"
        ? data.weight
        : convertWeight(data.weight, "lbs", "kg");
    ...
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          <div
            className={`rounded-full p-2 ${isEditMode ? "bg-blue-100" : "bg-blue-100"}`}
          >
            <Apple
              className={`w-5 h-5 ${isEditMode ? "text-blue-600" : "text-blue-600"}`}
            />
          </div>
          <span>
            {isEditMode
              ? `Edit Fruit Load #${loadSequence}`
              : `Add Fruit Load #${loadSequence}`}
          </span>
        </CardTitle>
      </CardHeader>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-blue-800">
                        {purchaseLines.summary.totalAvailableItems} available
                        purchase lines
                      </span>
                      <span className="text-blue-800 font-medium">
                        {convertWeight(
                          purchaseLines.summary.totalAvailableKg,
                          "kg",
                          "lbs",
                        ).toFixed(1)}{" "}
                        lbs total
                      ...
                      {filteredPurchaseLines.length > 0 ? (
                        filteredPurchaseLines.map((line) => (
                          <button
                            ...
                            className={`w-full p-4 text-left rounded-lg border transition-all touch-manipulation min-h-[44px] ${
                              selectedPurchaseItem?.purchaseItemId ===
                              line.purchaseItemId
                            ...
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <p className="text-xs text-gray-500">
                                  {line.originalQuantity} {line.originalUnit}{" "}
                                  purchased
                                  {line.harvestDate &&
                                    ` • Harvested ${line.harvestDate}`}
                                </p>
                              </div>
                              <div className="text-right">
                                <p className="text-sm font-medium text-green-600">
                                  {convertWeight(
                                    line.availableQuantityKg,
                                    "kg",
                                    "lbs",
                                  ).toFixed(1)}{" "}
                                  lbs available
                                ...
                                <p className="text-xs text-gray-500">
                                  {line.availablePercentage.toFixed(0)}%
                                  remaining
                                </p>
                              </div>
                            </div>
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-gray-600">
                            {searchQuery
                              ? "No purchase lines match your search"
                              : "No available purchase lines"}
                          </p>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-medium text-green-800">
                        Selected: {selectedPurchaseItem.varietyName} from{" "}
                        {selectedPurchaseItem.vendorName}
                      </span>
                    </div>
                  </div>
                )}
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <div className="flex items-center justify-center space-x-3">
                      ...
                      <span className="text-sm font-medium text-blue-900">
                        {getConvertedWeight().value.toFixed(1)}{" "}
                        {getConvertedWeight().unit}
                      </span>
                    </div>
                  </div>
                )}
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm text-blue-800">
                        Purchase estimate:{" "}
                        {parseFloat(
                          selectedPurchaseItem.quantityKg || "0",
                        ).toFixed(1)}{" "}
                        kg
                        <span className="text-blue-600 ml-1">
                          (actual pressing weight may vary)
                        </span>
                      </span>
                    </div>
                  </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Add any observations about fruit quality, pressing
                    conditions, etc.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
