...
export interface OptimizedSearchHook<T extends SearchableItem> {
  ...
  getPerformanceReport: () => ReturnType<SearchPerformanceMonitor['getPerformanceReport']>
}
...
export function useOptimizedSearch<T extends SearchableItem>(
  ...
  useEffect(() => {
    ...
    if (searchConfig.enablePerformanceMonitoring && !performanceMonitorRef.current) {
  ...
  const performSearch = useCallback(async (searchQuery: string): Promise<void> => {
    ...
    try {
      ...
      const { results: searchResults, metrics: searchMetrics } = await new Promise<{
        results: SearchResult<T>[]
      ...
      if (totalTime > 300) {
        console.warn(`Search took ${totalTime.toFixed(2)}ms (over 300ms target)`, {
          query: searchQuery,
...
export function useSearchHighlighting<T extends SearchableItem>(
  ...
  const getHighlightedText = useCallback((item: T, field: string): string | null => {
  ...
  const hasHighlights = useCallback((item: T): boolean => {
    const result = results.find(r => r.item.id === item.id)
