...
export function useTableSorting<T extends string = string>({
  ...
  const handleSort = useCallback(
    (field: T, forceDirection?: SortDirection) => {
      setSortState((prev) => {
        const existingColumnIndex = prev.columns.findIndex(
          (col) => col.field === field,
        );
        ...
        if (forceDirection !== undefined) {
          ...
          if (forceDirection === null) {
            ...
            const newColumns = prev.columns.filter(
              (col) => col.field !== field,
            );
            return {
              columns: newColumns,
        ...
        if (existingColumnIndex >= 0) {
          ...
          if (newDirection === null) {
            ...
            // Update direction
            const newColumns = [...prev.columns];
            const updatedColumn: SortColumn<T> = {
              field,
  ...
  const sortData = useCallback(
    <D>(data: D[], getSortValue?: (item: D, field: T) => any): D[] => {
