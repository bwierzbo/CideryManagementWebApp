...
export class PackagingPDFGenerator {
  ...
  private addHeader(data: PackagingRunPDFData): void {
    ...
    this.doc.text(
      this.options.companyName || "Cidery Management",
      centerX,
      this.currentY,
      { align: "center" },
    );
    ...
    if (this.options.companyAddress) {
      ...
      this.doc.text(this.options.companyAddress, centerX, this.currentY, {
        align: "center",
      });
      this.currentY += 8;
    }
    ...
    this.doc.setFont("helvetica", "bold");
    this.doc.text("PACKAGING RUN REPORT", centerX, this.currentY, {
      align: "center",
    });
    this.currentY += 15;
  ...
  private addProductionSummary(data: PackagingRunPDFData): void {
    ...
    this.doc.setFont("helvetica", "normal");
    this.doc.text(
      data.batch.name || `Batch ${data.batchId.slice(0, 8)}`,
      leftCol + 25,
      this.currentY,
    );
    ...
    this.doc.setFont("helvetica", "normal");
    this.doc.text(
      data.vessel.name || `Vessel ${data.vesselId.slice(0, 8)}`,
      leftCol + 25,
    ...
    this.doc.setFont("helvetica", "normal");
    this.doc.text(
      this.formatPackageSize(data.packageSizeML, data.packageType),
      leftCol + 25,
    ...
    this.doc.setFont("helvetica", "normal");
    this.doc.text(
      data.unitsProduced.toLocaleString(),
      rightCol + 35,
      this.currentY,
    );
  ...
  private addQAMeasurements(data: PackagingRunPDFData): void {
    ...
    this.doc.setFont("helvetica", "normal");
    this.doc.text(
      data.abvAtPackaging ? `${data.abvAtPackaging.toFixed(2)}%` : "Not tested",
      leftCol + 35,
      this.currentY,
    );
    ...
    this.doc.setFont("helvetica", "normal");
    this.doc.text(
      data.carbonationLevel || "Not specified",
      leftCol + 35,
      this.currentY + lineHeight,
    );
    ...
    this.doc.text(
      data.fillVarianceML
        ? `${data.fillVarianceML.toFixed(1)}ml`
        : "Not measured",
      rightCol + 30,
      this.currentY,
    );
  ...
  private addSectionHeader(title: string): void {
    ...
    this.doc.line(
      this.marginLeft,
      this.currentY,
      this.marginLeft + this.doc.getTextWidth(title),
      this.currentY,
    );
    this.currentY += 10;
  }
  ...
  private addFooter(): void {
    ...
    this.doc.text(pageNumber, this.pageWidth - this.marginRight, footerY, {
      align: "right",
    });
  }
